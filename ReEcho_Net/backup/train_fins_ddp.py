...
def train_loop(train_dl, val_dl, local_rank,
    ...
    rir_dec_net = RIRDecoder_Fins().to(device)

    # ------------ load checkpoint ------------
    checkpoint = torch.load(f"zoo/FiNs_joint_100.pth", map_location=device)
...
def main():
    ...
    train_loop(tr_dl, va_dl, local_rank, epochs=100, lr=1e-4)
