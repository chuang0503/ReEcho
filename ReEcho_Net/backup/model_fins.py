...
def get_bandpass_filters(sr=16000, bp_kernel_size=1023):
    ...
    center_freqs = octave(low_freq, high_freq/np.sqrt(3))
    num_filters = len(center_freqs)
    fir_torch = torch.zeros(num_filters, bp_kernel_size)
    for i, (low,high) in enumerate(zip(octave_low(low_freq, high_freq/np.sqrt(3)), octave_high(low_freq, high_freq/np.sqrt(3)))):
        fir_torch[i, :] = torch.from_numpy(firwin(bp_kernel_size, [low, high], window='hamming', fs=sr, pass_zero=False))
...
class RIREmbedding(nn.Module):
    ...
    def forward(self, x_in):
        ...
        seed = self.conv_last(emb.unsqueeze(2)) #(B, hidden, T=100)

        return emb, seed

class RIRDecoder(nn.Module):
    def __init__(self, input_dim=128, bp_kernel_size=1023, sr=16000, early_rir_duration=0.05):
        super(RIRDecoder, self).__init__()
        self.sr = sr
    ...
    def forward(self, seed, noise=None): # require B, C, T
...
# ─────────────────── test ───────────────────
def test_rir_decoder():
    rir_decoder = RIRDecoder(input_dim=256, sr=16000, early_rir_duration=0.05)
    seed = torch.randn(2, 256, 100)
    out, rir_ch, filtered_noise = rir_decoder(seed)
    print(out.shape, rir_ch.shape)
