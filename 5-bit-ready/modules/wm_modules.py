...
class WMEmbedderExtractor(nn.Module):
    ...
    def wm_extraction(self, reverb_spec, mode='logit'):
        emb_T = self.encoder(reverb_spec)
        emb = self.pooling(emb_T)
        pred_logit = self.extractor(emb) # (B, msg_len)
        if mode == 'logit':
            out = pred_logit
        elif mode == 'msg':
            out = (pred_logit>0.5).float() # (B, msg_len)
        else:
...
def test_wm():
    ...
    msg_out = wm.wm_extraction(reverb_spec, mode='logit')
    ic(msg_out)

    # extraction (test)
    msg_out = wm.wm_extraction(reverb_spec, mode='msg')
    ic(msg_out)

    # ber
    ber = (msg_out != msg).float().mean()
    ic(ber)
