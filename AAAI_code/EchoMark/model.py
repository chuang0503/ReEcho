...
class ReEcho_WM(nn.Module):
    ...
    def extraction(self, reverb_spec, mode='logit'):
        wm_exist, logit = self.wm_net.wm_extraction(reverb_spec, mode=mode)
        return wm_exist, logit
...
def test_ReEchoNet():
    ...
    wm_exist, msg_logit = watermarker.extraction(reverb_spec, mode='test')
    ic(msg_logit.shape)
    ic(msg_logit)
    ic(wm_exist)
    ic(msg_logit[wm_exist==0,:])
