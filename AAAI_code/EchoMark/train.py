...
from model import *
from new_dataloader import get_dataloader, MyLibriSpeech, RIRS_Dataset, BUT_Dataset, Noise_Dataset, add_noise
...
# ─────────────────── training loop ───────────────────
def train_loop(audio_dl_train, audio_dl_val, rir_dl_train, rir_dl_val, noise_dl_train, noise_dl_val, msg_len=5,
    ...
    for ep in range(1, epochs + 1):
        ...
        train_pbar = tqdm(zip(audio_dl_train, rir_dl_train, noise_dl_train), total=len(audio_dl_train), desc=f"Epoch {ep}/{epochs} [Train]")
        for audio, rir, noise in train_pbar:
            ...
            wm_logit, msg_logit = watermarker.extraction(rs_detect_spec, mode='train')
            # loss
        ...
        if ep % 20 == 0:
            ...
            with torch.no_grad():
                val_pbar = tqdm(zip(audio_dl_val, rir_dl_val, noise_dl_val), total=len(audio_dl_val), desc=f"Epoch {ep}/{epochs} [Val]")
                for audio, rir, noise in val_pbar:
                    ...
                    #
                    rs_detect_spec = spec_transform(rs_detect)
                    wm_logit, msg_logit = watermarker.extraction(rs_detect_spec, mode='test')
                    # loss
                    dereverb_loss = stft_loss(spec_masked, spec_transform(audio))
                    rir_loss = ms_loss(rir_est, rir) + edc_loss(rir_est, rir)
...
# ─────────────────── main ───────────────────
def main():
    audio_dataset_train = MyLibriSpeech(url="dev-clean",sr=16000, duration=2) # change to train-clean-100 when you're ready
    ...
    audio_dl_train, rir_dl_train, noise_dl_train = get_dataloader(audio_dataset_train, rir_train, noise_dataset, batch_size=40, num_workers=64, persistent_workers=True, pin_memory=True)
    audio_dl_val, rir_dl_val, noise_dl_val = get_dataloader(audio_dataset_val, rir_val, noise_dataset, batch_size=40, num_workers=64, persistent_workers=True, pin_memory=True)
    print("Starting training...")
    # just for test
    train_loop(audio_dl_train, audio_dl_val, rir_dl_train, rir_dl_val, noise_dl_train, noise_dl_val, msg_len=5, epochs=200, lr=1e-4)
